<?php
// +----------------------------------------------------------------------
// | YGWCMS [ WE CAN DO IT MORE SIMPLE ]
// +----------------------------------------------------------------------
// | Copyright (c) 2017-2018 http://www.yangblogs.com All rights reserved.
// +----------------------------------------------------------------------
// | Licensed ( http://www.yangblogs.com )
// +----------------------------------------------------------------------
// | Author: yang <601047564@qq.com>
// +----------------------------------------------------------------------
return [
    "USERNAME"                      => 'Username',
    "NICENAME"                      => 'Nicename',
    "AVATAR"                        => 'Avatar',
    "EMAIL"                         => 'Email',
    "REGISTRATION_TIME"             => 'Registration Time',
    "LAST_LOGIN_TIME"               => "Last LoginController Time",
    "LAST_LOGIN_IP"                 => 'Last LoginController IP',
    "STATUS"                        => 'Status',
    "ACTIONS"                       => "Actions",
    "USER_STATUS_BLOCKED"           => 'Blocked',
    "USER_STATUS_ACTIVATED"         => 'Activated',
    "USER_STATUS_UNVERIFIED"        => 'Unverified',
    "BLOCK_USER"                    => 'Block',
    "BLOCK_USER_CONFIRM_MESSAGE"    => 'Are you sure you want to pull black this user?',
    "ACTIVATE_USER"                 => 'Activate',
    "ACTIVATE_USER_CONFIRM_MESSAGE" => 'Are you sure you want to enable this user?',
    "THIRD_PARTY_USER"              => "Third Party User",
    "NOT_FILLED"                    => "Not Filled",
    'USER_INDEXADMIN_BAN'           => '拉黑会员',
    'USER_INDEXADMIN_CANCELBAN'     => '启用会员',
    'USER_INDEXADMIN_DEFAULT1'      => 'User Group',
    'USER_INDEXADMIN_DEFAULT3'      => 'Admin Group',
    'USER_INDEXADMIN_INDEX'         => 'Site Users',
    'USER_OAUTHADMIN_DELETE'        => '第三方用户解绑',
    'USER_OAUTHADMIN_INDEX'         => 'Third Party Users',
    'USER_INDEXADMIN_DEFAULT'       => 'Users',
    "USERNAME"                      => 'Username',
    "USER_FROM"                     => 'From',
    "AVATAR"                        => 'Avatar',
    "BINGDING_ACCOUNT"              => 'Binding Account',
    "FIRST_LOGIN_TIME"              => 'First LoginController Time',
    "LAST_LOGIN_TIME"               => "Last LoginController Time",
    "LAST_LOGIN_IP"                 => 'Last LoginController IP',
    "LOGIN_TIMES"                   => 'LoginController Times',
    "STATUS"                        => 'Status',
    "ACTIONS"                       => "Actions"
];